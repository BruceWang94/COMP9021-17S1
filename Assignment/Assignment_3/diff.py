...
class DiffCommands(object):
    ...
    def __init__(self, filename = None):
        ...
        if match:
            ...
            for ch in range(len(filename) - 1):
                self.value += filename[ch]
...
class OriginalNewFiles(object):
    ...
    def is_a_possible_diff(self, diff): # diff is a string
        ...
        print ('file1 has %d lines, file2 has %d lines' %(len_of_file_1,len_of_file_2))
        print ('file1 is',self.file_1)
        print ('file2 is',self.file_2)
        print ('',list_1, '\n', list_2)
        ...
        for i in range(1, length):
            ...
            if (list_1[i] - 1) > len_of_file_1 or (list_2[i] - 1) > len_of_file_2:
                return (False)
                break
    ...
    def output_unmodified_from_original(self, diff):
        ...
        len_of_file_1 = len(self.file_1)
        if len_of_file_1 != file_1_list[-1]:
            print ('...')
    ...
    def output_unmodified_from_new(self, diff):
        ...
        len_of_file_2 = len(self.file_2)
        if len_of_file_2 != file_2_list[-1]:
            print ('...')
        ...
        if type(list1[0]) == int:
            ...
            for j in range(possible):
                ...
                if f1_start == len_of_file_1 and f2 != len_of_file_2: # num1 a num2
                    ...
                    if diff_1 == 1: # num1 c num3,num4
                        txts[j] += str(f1_start + 1) + 'c' + str(f2_start) + ',' + str(len_of_file_2) + '\n'
                    elif diff_1 == 1: # num1,num2 c num3
                        txts[j] += str(f1_start + 1) + ',' + str(len_of_file_1) + 'c' + str(f2_start) + '\n'
                    else: # num1,num2 d num3,num4
                        txts[j] += str(f1_start + 1) + ',' + str(len_of_file_1) + 'c' + str(f2_start) + ',' + str(len_of_file_2) + '\n'
                #print(str(txts[j]))
...
if __name__ == '__main__':
    ...
    print ('>>> diff_1 = DiffCommands(\'diff_1.txt\')')
    diff_1 = DiffCommands('diff_1.txt')

    print('>>> print(diff_1)')
    print(diff_1)

    print ('>>> diff_2 = DiffCommands(\'diff_2.txt\')')
    diff_2 = DiffCommands('diff_2.txt')

    print('>>> print(diff_2)')
    print(diff_2)

    print ('>>> diff_3 = DiffCommands(\'diff_3.txt\')')
    ...
    print ('>>> pair_of_files.output_diff(diff_1)')
    pair_of_files.output_diff(diff_1)

    print ('>>> pair_of_files.output_unmodified_from_original(diff_1)')
    pair_of_files.output_unmodified_from_original(diff_1)

    print ('>>> pair_of_files.output_unmodified_from_new(diff_1)')
    ...
    '''
    print('>>> diff_1 = DiffCommands(\'wrong_1.txt\')')
    diff_1 = DiffCommands('wrong_1.txt')
